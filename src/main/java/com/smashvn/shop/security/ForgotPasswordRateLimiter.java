...
public class ForgotPasswordRateLimiter {
    ...
    // Lưu số lần yêu cầu sai/gửi liên tục
    ...
    public boolean isBlocked(String key) {
        if (lockTimeCache.containsKey(key)) {
            ...
            if (System.currentTimeMillis() - lockTime < BLOCK_DURATION_MS) {
                ...
                log.info("Mở khóa rate-limit quên mật khẩu thành công cho IP: {}", key);
            }
        }
        return false;
    }
    
    public void forgotPasswordFailed(String key) {
        int attempts = attemptsCache.getOrDefault(key, 0) + 1;
        attemptsCache.put(key, attempts);
        log.warn("Yêu cầu quên mật khẩu thất bại lần {} cho IP: {}", attempts, key);
        
        if (attempts >= MAX_ATTEMPTS) {
            lockTimeCache.put(key, System.currentTimeMillis());
            // Log an ninh dạng WARN
            log.warn("[SECURITY_EVENT] FORGOT_PASSWORD_RATE_LIMIT_TRIGGER: IP: {} bị chặn yêu cầu quên mật khẩu do vượt quá {} lần thử.", key, MAX_ATTEMPTS);
        }
    }
    
    public void forgotPasswordSucceeded(String key) {
        attemptsCache.remove(key);
        lockTimeCache.remove(key);
    }
}
