...
> **Lần cập nhật cuối:** 03/07/2026 — 16:23 (GMT+7)
...
| **02/07/2026 09:00** | — (UX Redesign)      | **Đã hoàn thành**     | Tinh chỉnh bố cục và kích thước Header Admin: (1) Tách Header Admin thành 2 wrapper phân đoạn `.admin-header-top-section` và `.admin-header-bottom-section` có độ rộng 100% viewport; (2) Thêm đường kẻ ngang full-width `border-bottom: 1px solid #f1f5f9` trên top wrapper; (3) Tăng kích thước logo từ 60px lên 80px; (4) Giảm độ rộng tối đa thanh tìm kiếm từ 440px xuống 380px; (5) Giảm cỡ chữ các tab menu từ `0.88rem` xuống `12px` với padding `8px 12px` đồng bộ tuyệt đối với phong cách khách hàng. | Antigravity AI      |
| **02/07/2026 09:17** | — (UX Polish)        | **Đã hoàn thành**     | Thay đổi cột ID thành STT trên trang Dashboard: (1) Đổi tên tiêu đề cột từ "ID" sang "STT" cho cả 3 bảng (Tài khoản nhân viên, Tài khoản khách hàng, Danh sách sản phẩm gốc); (2) Sử dụng biến lặp `iterStat.count` của Thymeleaf để hiển thị số thứ tự tăng dần từ 1 cho cả 3 bảng thay vì hiển thị mã ID cơ sở dữ liệu gốc giúp người dùng dễ đếm và quản lý trực quan. |
| **03/07/2026 14:42** | — (Bug Fix)          | **Đã hoàn thành**     | Sửa lỗi khi nhấn nút Đăng ký nhận ưu đãi (Newsletter): (1) Thêm `/api/newsletter/subscribe` vào danh sách ngoại lệ CSRF của Spring Security (`SecurityConfig.java`) để cho phép khách vãng lai gửi email; (2) Tối ưu hóa AJAX fetch ở `footer.html` kiểm tra Content-Type của phản hồi trước khi parse JSON để tránh gặp lỗi cú pháp khi máy chủ phản hồi trang lỗi HTML. | Antigravity AI      |
| **03/07/2026 15:18** | — (Database Seeding) | **Đã hoàn thành**     | Tạo và thực thi mã Seed CSDL (`SeedDatabaseTest.java`): (1) Tự động chèn các cấu hình ban đầu (Trạng thái giỏ hàng, Phương thức thanh toán COD/ZaloPay/SePay); (2) Thêm mới 5 danh mục và 4 thương hiệu phổ biến; (3) Tạo 3 tài khoản mặc định được mã hóa mật khẩu BCrypt tương ứng các vai trò (QL: admin@smashvn.com, NV: staff@smashvn.com, KH: customer@smashvn.com); (4) Thêm 5 sản phẩm vợt và giày cầu lông với 7 biến thể chi tiết có mô tả tiếng Việt chuẩn Unicode. | Antigravity AI      |
| **03/07/2026 15:25** | — (Bug Fix & Seeding) | **Đã hoàn thành**     | Giải quyết lỗi giao diện Tin tức trống và cấu hình ảnh: (1) Copy các tệp ảnh blog mẫu (`post-1.jpg`, `post-2.jpg`, `post-3.jpg`) vào đúng thư mục vật lý `uploads/blog` của dự án; (2) Tích hợp thêm logic khởi tạo bài viết mẫu ở `SeedDatabaseTest.java` chèn 3 tin tức cầu lông thực tế với trạng thái `PUBLISHED` và liên kết ảnh hợp lệ ở `/uploads/blog/`, giúp màn hình trang chủ hiển thị tin tức đẹp mắt. | Antigravity AI      |
| **03/07/2026 15:30** | — (Bug Fix & Seeding) | **Đã hoàn thành**     | Sửa lỗi ảnh sản phẩm bị lỗi hiển thị ở trang chủ (NoResourceFoundException): (1) Cập nhật lại tên file ảnh của các sản phẩm đã seed ở `SeedDatabaseTest.java` khớp chính xác với các tệp ảnh thực tế đang có sẵn trong thư mục `uploads/product/` (Yonex Astrox 100ZZ, Yonex Arcsaber 11 Pro, Lining Axforce 100, Victor DriveX 12, Giày Yonex); (2) Bổ sung logic tự động cập nhật lại ảnh khi bản ghi đã tồn tại trong phương thức `seedVariant()`; (3) Chạy kiểm thử thành công để đồng bộ cập nhật lại toàn bộ đường dẫn ảnh đúng trên Cơ sở dữ liệu. | Antigravity AI      |
| **03/07/2026 15:52** | — (File Cleanup)     | **Đã hoàn thành**     | Tự động dọn dẹp các tệp ảnh lỗi và ảnh không sử dụng: (1) Bổ sung phương thức `cleanupImages()` trong `SeedDatabaseTest.java` để đối chiếu danh sách ảnh trong CSDL (bảng HinhAnhSanPham, Blog, DanhGiaAnh) với các file trong thư mục `uploads/`; (2) Lọc và xóa các file ảnh bị hỏng/trống (dung lượng <= 100 bytes) hoặc không có liên kết trong CSDL; (3) Quét dọn thành công, loại bỏ hàng trăm file ảnh rác và ảnh hỏng (đã xóa 134 ảnh đánh giá lỗi) giải phóng không gian lưu trữ cho dự án. | Antigravity AI      |
| **03/07/2026 16:23** | — (UX Polish & Seed)  | **Đã hoàn thành**     | Sửa lỗi ảnh mô tả và danh mục của Giày Yonex Power Cushion 65Z3 C1: (1) Sử dụng `generate_image` tạo 2 ảnh giày chất lượng cao thật (`yonex_65z3_white_blue.png`, `yonex_65z3_black_orange.png`) và sao chép vào `uploads/product/` để thay thế ảnh vợt Kawasaki lỗi; (2) Chạy mã Seed cập nhật lại đường dẫn ảnh mới trên CSDL; (3) Chỉnh sửa `product-detail.html` thay thế Breadcrumb danh mục tĩnh "Vợt Cầu Lông" bằng dữ liệu động để hiển thị đúng danh mục thực tế của sản phẩm (Giày cầu lông). | Antigravity AI      |
